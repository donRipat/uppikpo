...
def insert_rows_into_processed_data(connector: StoreConnector, dataframe: DataFrame, filename: str):
    ...
    connector.start_transaction()
    for row in rows:
        connector.execute(f'INSERT INTO data (airline_name, cabin_flown, overall_rating, '
                          f'source_file) VALUES (\'{row["airline_name"]}\', \'{row["cabin_flown"]}\', \'{row["overall_rating"]}\',1)')
    connector.end_transaction()


def update_rows_into_processed_data(connector: StoreConnector, dataframe: DataFrame, filename: str):
    rows = dataframe.to_dict('records')
    connector.start_transaction()
    for row in rows:
        connector.execute(f'UPDATE data set overall_rating = 10 where overall_rating > 8 ')
    connector.end_transaction()
