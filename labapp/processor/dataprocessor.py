...
class DataProcessor(ABC):
    ...
    def sort_data_by_name(self, df, colname) -> pandas.DataFrame:
       # return df.groupby(['airline_name', 'cabin_flown']).agg({colname: 'mean'}).to_markdown()
       return df.groupby(["airline_name", "cabin_flown"], as_index=False)[colname].mean()
...
class CsvDataProcessor(DataProcessor):
    ...
    def read(self):
        try:
            self._dataset = pandas.read_csv(self._datasource, sep=self.separator, header='infer', names=None, encoding='latin-1')
    ...
    #    Сортируем данные по значениям колонки "date" и сохраняем результат в атрибуте result
    def run(self):
        self.result = self.sort_data_by_col(self._dataset, "date", True)

    def average(self):
        self.result = self.sort_data_by_name(self._dataset, "overall_rating")
